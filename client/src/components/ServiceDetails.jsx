...
import { applyJob, showService } from '../Redux/FreelancerSlice';
...
export default function ServiceDetails({ type }) {
    ...
    const handleApply = () => {
        setLoading(true)
        dispatch(applyJob(serviceId)).unwrap().then(data => {
            setTimeout(() => {
                setLoading(false)
                if (data.status == 200) {
                    toast.success(data.msg)
                                ...
                                <>
                                    {
                                        ...
                                        <>
                                            ...
                                            <div className="bottom-buttons">
                                                {
                                                    data.user.role==='client'? <> <button onClick={handleApply}>Apply</button>
                                                    <HashLink className="go-back-button" to={`/dashboard/freelancer/${id}/jobs`}><button>Go Back</button></HashLink>
