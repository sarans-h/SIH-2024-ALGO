...
const findFreeApplied = async (freeId) => {
  const selectedClient = await findUserById(freeId);
  if (selectedClient) {
    if (selectedClient.role != "freelancer") {
      return "You Don't Have Permission";
    }
    const freeApplied = await orderModel.find({ freeId }).sort({ updatedAt: -1 });
    if (freeApplied.length != 0) {
      let allOrdersInfo = [];
      for (let i of freeApplied) {
